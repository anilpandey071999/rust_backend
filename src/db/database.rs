...
impl Database {
    ...
    pub async fn add_pizza(&self, new_pizza: Pizza) -> Option<Pizza> {
        let create_pizza = self.client.create(("pizza", new_pizza.uuid.clone())).content(new_pizza).await;
        match create_pizza {
            Ok(created) => created,
            Err(_) => None,
        }
    ...
    pub async fn update_pizza(&self, uuid: String, pizza_name: String) -> Option<Pizza> {
        ...
        match find_pizza {
            Ok(found) => {
                match found {
                    Some(_found_pizza) => {
                        let update_pizza: Result<Option<Pizza>, Error> = self.client.update(("pizza", &uuid)).merge(Pizza{
                            uuid, 
